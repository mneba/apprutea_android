...
export default function ClientesScreen({ navigation, route }: any) {
  ...
  const loadLiq = useCallback(async () => {
    ...
    try {
      ...
      const ps = new Set<string>(); const pm = new Map<string, PagamentoParcela>();
      (d2 || []).forEach((p: PagamentoParcela) => { ps.add(p.parcela_id); pm.set(p.parcela_id, p); });
      setRaw(d1 || []); setPagasSet(ps); setPagMap(pm);
    } catch (err) { console.error('Erro load liq:', err); }
  }, [rotaId, dataLiq]);

  // ── Load Todos (com dados completos dos empréstimos) ──
  const loadTodosF = useCallback(async () => {
    ...
    try {
      ...
      // Parcelas vencidas por empréstimo
      const empIds = (data || []).map((e: any) => e.id);
      const vencMap = new Map<string, { total: number; valor: number }>();
      ...
      if (empIds.length > 0) {
        ...
        (pVenc || []).forEach((p: any) => {
          ...
          ex.total += 1; ex.valor += p.valor_parcela || 0;
          vencMap.set(p.emprestimo_id, ex);
        });
        (pMax || []).forEach((p: any) => {
          if (!parcAtualMap.has(p.emprestimo_id)) parcAtualMap.set(p.emprestimo_id, p.numero_parcela + 1);
        });
      }

      const mapa = new Map<string, ClienteTodos>();
      (data || []).forEach((e: any) => {
        ...
        const empT: EmprestimoTodos = {
          ...
          total_parcelas_vencidas: venc.total, valor_total_vencido: venc.valor,
        };
        const ex = mapa.get(c.id);
        if (!ex) {
          mapa.set(c.id, { id: c.id, consecutivo: c.consecutivo, nome: c.nome, telefone_celular: c.telefone_celular, status: c.status, tem_atraso: e.status === 'VENCIDO' || venc.total > 0, emprestimos: [empT] });
        } else {
          ex.emprestimos.push(empT);
          if (e.status === 'VENCIDO' || venc.total > 0) ex.tem_atraso = true;
        }
      });
      setTodosList(Array.from(mapa.values()).sort((a, b) => a.nome.localeCompare(b.nome)));
    } catch (err) { console.error('Erro load todos:', err); }
    finally { setLoadTodos(false); }
  }, [rotaId]);

  const loadAll = useCallback(async () => { setLoading(true); await Promise.all([loadLiq(), loadTodosF()]); setLoading(false); }, [loadLiq, loadTodosF]);
  useEffect(() => { loadAll(); }, [loadAll]);
  const onRefresh = useCallback(() => { setRefreshing(true); loadAll().finally(() => setRefreshing(false)); }, [loadAll]);

  // ── Agrupamento Liquidação ──
  ...
  const renderCard = (c: ClienteAgrupado) => {
    ...
    return (
      <TouchableOpacity key={c.cliente_id} activeOpacity={0.7} onPress={() => setExpanded(p => p === c.cliente_id ? null : c.cliente_id)} style={[S.card, { borderLeftColor: bc, backgroundColor: bg }]}>
        <View style={S.cardRow}>
          ...
          <View style={S.cardInfo}>
            <View style={S.nameRow}>
              ...
              {e.tem_parcelas_vencidas && e.total_parcelas_vencidas > 0 && <View style={S.bWarn}><Text style={S.bWarnI}>⚠</Text><Text style={S.bWarnT}>{e.total_parcelas_vencidas}</Text></View>}
              {c.tem_multiplos_vencimentos && <View style={S.bMul}><Text style={S.bMulT}>{c.qtd_emprestimos}</Text></View>}
        ...
        <View style={S.pRow}>
          <View>
            <View style={S.pLblR}><Text style={S.pLbl}>{t.parcela} {e.numero_parcela}/{e.numero_parcelas}</Text><View style={S.fBdg}><Text style={S.fBdgT}>{FREQ[e.frequencia_pagamento] || e.frequencia_pagamento}</Text></View></View>
            {pg && pi ? (<><Text style={S.pgVal}>{t.pago} {fmt(pi.valorPago)}</Text><Text style={S.pgOrig}>{t.original} {fmt(pi.valorParcela)}</Text>{pi.creditoGerado > 0 && <Text style={S.pgCred}>{t.credito} {fmt(pi.creditoGerado)}</Text>}</>) : (<Text style={S.pVal}>{fmt(e.valor_parcela)}</Text>)}
          </View>
        ...
        {ex && (
          <View style={S.exp}>
            ...
            <View style={S.res}>
              <View style={S.resH}><Text style={S.resT}>{t.emprestimo} {ei + 1}/{c.qtd_emprestimos}</Text><View style={[S.stB, { backgroundColor: e.status_dia === 'EM_ATRASO' ? '#FEE2E2' : pg ? '#D1FAE5' : '#F3F4F6' }]}><Text style={[S.stBT, { color: e.status_dia === 'EM_ATRASO' ? '#DC2626' : pg ? '#059669' : '#6B7280' }]}>{pg ? 'PAGO' : e.status_dia}</Text></View></View>
              <View style={S.g3}><View style={S.gi}><Text style={S.gl}>{t.principal}</Text><Text style={S.gv}>{fmt(e.valor_principal)}</Text></View><View style={S.gi}><Text style={S.gl}>{t.juros}</Text><Text style={[S.gv, { color: '#F59E0B' }]}>{fmt(juros)}</Text></View><View style={S.gi}><Text style={S.gl}>{t.total}</Text><Text style={S.gv}>{fmt(totalE)}</Text></View></View>
              <View style={S.g3}><View style={S.gi}><Text style={S.gl}>{t.jaPago}</Text><Text style={[S.gv, { color: '#10B981' }]}>{fmt(totalE - e.saldo_emprestimo)}</Text></View><View style={S.gi}><Text style={S.gl}>{t.saldo}</Text><Text style={[S.gv, { color: '#EF4444' }]}>{fmt(e.saldo_emprestimo)}</Text></View><View style={S.gi}><Text style={S.gl}>{t.parcelas}</Text><Text style={S.gv}>{pp}/{e.numero_parcelas}</Text></View></View>
              <Text style={S.prL}>{t.progresso}</Text><View style={S.prB}><View style={[S.prF, { width: `${pct}%` }]} /></View><Text style={S.prR}>{pr} {t.restantes}</Text>
            </View>
            <View style={S.btR}>
              <TouchableOpacity style={[S.bt, S.btG, (isViz || pg) && S.btOff]} onPress={() => { if (!isViz) navigation.navigate('Pagamento', { clienteId: c.cliente_id, clienteNome: c.nome, parcelaId: e.parcela_id, emprestimoId: e.emprestimo_id, valorParcela: e.valor_parcela, saldoParcela: e.saldo_parcela, numeroParcela: e.numero_parcela, totalParcelas: e.numero_parcelas, liquidacaoId: liqId, rotaId }); }} disabled={isViz || pg}><Text style={S.btI}>💰</Text><Text style={S.btW}>{t.pagar}</Text></TouchableOpacity>
              <TouchableOpacity style={[S.bt, S.btBl]} onPress={() => navigation.navigate('Parcelas', { emprestimoId: e.emprestimo_id })}><Text style={S.btI}>👁</Text><Text style={S.btW}>{t.verParcelas}</Text></TouchableOpacity>
            </View>
            <View style={S.btR}>
              <TouchableOpacity style={[S.bt, S.btOG]} onPress={() => c.telefone_celular && Linking.openURL(`tel:${c.telefone_celular.replace(/\D/g, '')}`)} disabled={!c.telefone_celular}><Text style={S.btI}>📱</Text><Text style={S.btTG}>{t.contato}</Text></TouchableOpacity>
              <TouchableOpacity style={[S.bt, S.btOB]} onPress={() => { if (c.latitude && c.longitude) Linking.openURL(Platform.OS === 'ios' ? `maps:?daddr=${c.latitude},${c.longitude}` : `google.navigation:q=${c.latitude},${c.longitude}`); }} disabled={!c.latitude}><Text style={S.btI}>🧭</Text><Text style={S.btTB}>{t.ir}</Text></TouchableOpacity>
            </View>
          </View>)}
      </TouchableOpacity>);
  };
  ...
  const renderTodos = (c: ClienteTodos) => {
    ...
    return (
      <TouchableOpacity key={c.id} activeOpacity={0.7} onPress={() => setExpandedTodos(p => p === c.id ? null : c.id)} style={[S.card, { borderLeftColor: cor }]}>
        <View style={S.cardRow}>
          <View style={[S.av, { backgroundColor: cor }]}><Text style={S.avTx}>{getIni(c.nome)}</Text></View>
          <View style={S.cardInfo}>
            <View style={S.nameRow}>
              <Text style={S.nome} numberOfLines={1}>{c.nome.toLowerCase()}</Text>
              {a && <Text style={[S.tSt, { color: '#EF4444' }]}>{t.statusAtraso}</Text>}
              <Text style={S.dots}>⋮</Text>
            </View>
          </View>
        </View>
        {ex && emp && (
          <View style={S.exp}>
            ...
            <View style={S.tEmpCard}>
              <View style={S.tEmpHead}><Text style={S.tEmpTitle}>{isVencido ? t.empVencido : t.empAtivo}</Text><Text style={S.tEmpParcela}>{emp.numero_parcela_atual}/{emp.numero_parcelas}</Text></View>
              <View style={S.tEmpBody}>
                <View><Text style={S.tEmpLbl}>{t.valorParcela}</Text><Text style={S.tEmpVal}>{fmt(emp.valor_parcela)}</Text></View>
            ...
            {c.emprestimos.length > 1 && (
              <View style={S.eNav}>
                <TouchableOpacity onPress={() => setEmpIdxTodos(p => ({ ...p, [c.id]: Math.max(0, ei - 1) }))} disabled={ei === 0} style={[S.eNBtn, ei === 0 && S.eNOff]}><Text style={S.eNBTx}>◀</Text></TouchableOpacity>
                {c.emprestimos.map((_, i) => <View key={i} style={[S.eDot, i === ei && S.eDotOn]} />)}
                <TouchableOpacity onPress={() => setEmpIdxTodos(p => ({ ...p, [c.id]: Math.min(c.emprestimos.length - 1, ei + 1) }))} disabled={ei >= c.emprestimos.length - 1} style={[S.eNBtn, ei >= c.emprestimos.length - 1 && S.eNOff]}><Text style={S.eNBTx}>▶</Text></TouchableOpacity>
            ...
            <View style={S.btR}>
              <TouchableOpacity style={[S.bt, S.btRed]} onPress={() => c.telefone_celular && Linking.openURL(`tel:${c.telefone_celular.replace(/\D/g, '')}`)} disabled={!c.telefone_celular}><Text style={S.btI}>💬</Text><Text style={S.btW}>{t.contato}</Text></TouchableOpacity>
              <TouchableOpacity style={[S.bt, S.btBl]} onPress={() => navigation.navigate('Parcelas', { emprestimoId: emp.id })}><Text style={S.btI}>👁</Text><Text style={S.btW}>{t.detalhes}</Text></TouchableOpacity>
            </View>
          </View>)}
      </TouchableOpacity>);
  };
  ...
  return (
    <View style={S.c}>
      ...
      <View style={S.srR}>
        <View style={S.srB}><Text style={S.srI}>🔍</Text><TextInput style={S.srIn} placeholder={t.buscar} placeholderTextColor="#9CA3AF" value={busca} onChangeText={setBusca} /></View>
        {tab === 'liquidacao' && <TouchableOpacity style={S.orB} onPress={() => setShowOrd(!showOrd)}><Text style={S.orI}>↕️</Text><Text style={S.orTx}>{ord === 'rota' ? t.ordemRota : t.ordemNome}</Text><Text style={S.orCh}>▼</Text></TouchableOpacity>}
      </View>
      ...
      {tab === 'liquidacao' && (
        <View style={S.chs}>
          <TouchableOpacity style={[S.ch, filtro === 'todos' && S.chOn]} onPress={() => setFiltro('todos')}><Text style={[S.chTx, filtro === 'todos' && S.chTxOn]}>{t.filtroTodos} {filtered.length}</Text></TouchableOpacity>
          <TouchableOpacity style={[S.ch, filtro === 'atrasados' && S.chOn]} onPress={() => setFiltro('atrasados')}><Text style={[S.chTx, filtro === 'atrasados' && S.chTxOn]}>{t.filtroAtrasados} {cntAtraso}</Text></TouchableOpacity>
          <TouchableOpacity style={[S.ch, mostrarPagas ? S.chPOn : S.chPOff]} onPress={() => setMostrarPagas(!mostrarPagas)}><Text style={[S.chTx, mostrarPagas ? S.chPTxOn : S.chPTxOff]}>{t.filtroPagas} {cntPagas}</Text></TouchableOpacity>
